...
class Device(models.Model):
    ...
    unique_name = models.CharField(max_length=255, unique=True) #unique- suggested form of 'Org-Deployment-DeviceID', aka 'ML.LA.0001' for MediaLab's LearnAir Deployment device 1
    ...
    manufacture_date = models.DateTimeField(blank=True)
    deploy_date = models.DateTimeField(blank=True)
...
class SensorType(models.Model):
    ...
    #should be this, django upgrade to 1.8
...
class SensorData(models.Model):
    ...
    timestamp = models.DateTimeField(default=timezone.now, blank=True,
                                     db_index=True)
    #should be this, upgrade to django 1.8
...
class APIDataStore(models.Model):
    ...
    fixed_site = models.ForeignKey(FixedSite, related_name='api_datastore', null=True, blank=True) #should have either this or deployment
...
class APIData(models.Model):
    '''API Data value'''
    api_store = models.ForeignKey(APIDataStore, related_name='api_scalar_data')
    ...
    class Meta:
        ...
        index_together = [['api_store', 'timestamp']]

    def __repr__(self):
        return 'APIScalarData(timestamp=%r, duration=%r, value=%r, datastore=%r)' % (
            self.timestamp, self.duration_sec, self.value, self.api_store)

    def __str__(self):
        return '%.3f %s' % (self.value, self.api_store.unit)
...
class CalibrationData(models.Model):
    '''API Data value'''
    calibration_store = models.ForeignKey(CalibrationDataStore, related_name='calibration_data')
    timestamp = models.DateTimeField(default=timezone.now, blank=True,
                                     db_index=True)
    ...
    class Meta:
        ...
        index_together = [['calibration_store', 'timestamp']]

    def __repr__(self):
        return 'CalibrationData(timestamp=%r, value=%r, datastore=%r)' % (
            self.timestamp, self.value, self.calibration_store)

    def __str__(self):
        return '%.3f %s' % (self.value, self.calibration_store.unit)
